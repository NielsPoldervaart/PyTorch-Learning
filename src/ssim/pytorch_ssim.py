...
img1 = (
    transform(Image.open("./images/image1.jpg").convert("RGB")).unsqueeze(0).to(device)
)
img2 = (
    transform(Image.open("./images/image2.jpg").convert("RGB")).unsqueeze(0).to(device)
)
...
ms_ssim_val = ms_ssim(img1, img2, data_range=255, size_average=True)
...
ms_ssim_val = ms_ssim_module(img1, img2)
print(f"MS_SSIM: {ms_ssim_val.item() * 100:.2f}%")
